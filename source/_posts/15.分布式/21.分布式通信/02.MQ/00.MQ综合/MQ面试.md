...
- **强一致性** - 在 Producer 端设置 `acks=all`，这意味着：要求每条数据，必须是**写入所有 replica 之后，才能认为写入成功了**。保证强一致性需要付出一定的代价，通常只有业务场景真的需要保证万无一失才会这么设置。
- **失败重试** - 在 Producer 端设置 `retries=MAX`（很大很大很大的一个值，无限次重试的意思），这意味着**要求一旦写入失败，就无限重试**，卡在这里了。
