...
- [1. Raft 简介](#1-raft-简介)
  - [1.1. 分布式一致性](#11-分布式一致性)
  - [1.2. 复制状态机](#12-复制状态机)
  - [1.3. RAFT 应用](#13-raft-应用)
- [2. Raft 基础](#2-raft-基础)
  - [2.1. 服务器角色](#21-服务器角色)
  - [2.2. 任期](#22-任期)
  - [2.3. RPC](#23-rpc)
- [3. 选举 Leader](#3-选举-leader)
  - [3.1. 选举规则](#31-选举规则)
  - [3.2. 单 Candidate 选举](#32-单-candidate-选举)
  - [3.3. 多 Candidate 选举](#33-多-candidate-选举)
- [4. 日志复制](#4-日志复制)
  - [4.1. 日志格式](#41-日志格式)
  - [4.2. 日志复制流程](#42-日志复制流程)
  - [4.3. 日志一致性](#43-日志一致性)
- [5. 安全性](#5-安全性)
  - [5.1. 选举限制](#51-选举限制)
  - [5.2. 提交旧任期的日志条目](#52-提交旧任期的日志条目)
- [6. 日志压缩](#6-日志压缩)
- [7. 参考资料](#7-参考资料)
...
（1）下图表示一个分布式系统的最初阶段，此时只有 Follower，没有 Leader。Follower A 等待一个随机的**选举超时时间**（election timeout）之后，没收到 Leader 发来的心跳消息。因此，将 Term 由 0 增加为 1，转换为 Candidate，进入选举状态。
...
- [Raft 动画教程](http://thesecretlivesofdata.com/raft) - 通过动画逐步引导理解 Raft
- [Raft 交互式动画](https://raft.github.io/) - 可以测试各种场景下的 Raft 行为
